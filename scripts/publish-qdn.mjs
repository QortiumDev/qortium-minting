...
const nodeApiUrl = (process.env.QORTIUM_MINTING_NODE_API_URL ?? DEFAULT_NODE_API_URL).replace(/\/+$/, '');
const publishName = process.env.QORTIUM_MINTING_QDN_NAME ?? DEFAULT_NAME;
const identifier = process.env.QORTIUM_MINTING_QDN_IDENTIFIER ?? DEFAULT_IDENTIFIER;
const publishTitle = process.env.QORTIUM_MINTING_QDN_TITLE ?? DEFAULT_TITLE;
const service = process.env.QORTIUM_MINTING_QDN_SERVICE ?? 'APP';
const distPath = path.resolve(repoRoot, process.env.QORTIUM_MINTING_DIST_PATH ?? 'dist');
const apiKeyPath = expandHomePath(
  process.env.QORTIUM_MINTING_NODE_API_KEY_PATH ?? '~/.config/qortium-core/runtime/apikey.txt',
);
const previewAccountsPath = expandHomePath(
  process.env.QORTIUM_MINTING_PREVIEW_ACCOUNTS_PATH ??
...
function getApiKeySource() {
  const explicitApiKey = process.env.QORTIUM_MINTING_NODE_API_KEY?.trim();
  ...
  if (process.env.QORTIUM_MINTING_NODE_API_KEY_PATH?.trim()) {
    return {
...
async function ensureNameRegistered(name, account) {
  ...
  const rawRegisterBytes58 = buildRegisterNameRawBytes58({
    ...
    data: JSON.stringify({
      app: 'Minting',
      purpose: 'QDN minting status app preview',
    }),
  });
...
async function publishResource(account) {
  ...
  const rawUnsignedBytes58 = await request(
    appendQuery(resourcePathname, {
      title: publishTitle,
      description: 'QDN minting status app for Qortium Home',
      fee: 0,
    }),
    {
      method: 'POST',
