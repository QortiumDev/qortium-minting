...
describe('copyTextToClipboard', () => {
  it('uses navigator.clipboard when the QDN view permits it', async () => {
    ...
    await expect(copyTextToClipboard('Qaddress', dependencies)).resolves.toBe(true);
    expect(writeText).toHaveBeenCalledWith('Qaddress');
  });
  ...
  it('falls back to a selected textarea when navigator.clipboard is rejected', async () => {
    ...
    await expect(copyTextToClipboard('fallback text', dependencies)).resolves.toBe(true);
